...
export const pivotTests: TestSuite = {
  ...
  tests: [
    ...
    },
    {
      ...
      input:
        "select * from t pivot (sum(x) as s, avg(y) as a for col in ('a', 'b'))",
      expected:
        "SELECT * FROM t PIVOT (SUM(x) AS s, AVG(y) AS a FOR col IN ('a', 'b'))",
    },
    {
      ...
      input:
        'select * from sales_data pivot (sum(revenue) as total_rev, avg(revenue) as avg_rev, count(*) as cnt for quarter_name in (Q1_2023, Q2_2023, Q3_2023, Q4_2023))',
      expected:
    ...
    },
    {
      ...
      input:
        'select * from t pivot (sum(val) for category in (category_one, category_two, category_three, category_four, category_five, category_six, category_seven, category_eight, category_nine))',
      expected:
  ...
  tests: [
    {
      ...
      input:
        'select * from t unpivot (val for col in (a as col_a, b as col_b))',
      expected:
        'SELECT * FROM t UNPIVOT (val FOR col IN (a AS col_a, b AS col_b))',
    },
  ],
...
export const lateralViewTests: TestSuite = {
  ...
  tests: [
    ...
    },
    {
      ...
      expected:
        'SELECT * FROM t LATERAL VIEW EXPLODE(arr) t_alias AS col_alias',
    },
    {
    ...
    },
    {
      ...
      input:
        'select * from t lateral view explode(arr1) as a lateral view explode(arr2) as b',
      expected:
        'SELECT * FROM t LATERAL VIEW EXPLODE(arr1) AS a LATERAL VIEW EXPLODE(arr2) AS b',
    },
  ],
...
export const stackTests: TestSuite = {
  ...
  tests: [
    ...
    },
    {
      ...
      input:
        "select stack(5, 'very_long_alias_name_one', col1, 'very_long_alias_name_two', col2, 'very_long_alias_name_three', col3, 'very_long_alias_name_four', col4, 'very_long_alias_name_five', col5) from t",
      expected:
